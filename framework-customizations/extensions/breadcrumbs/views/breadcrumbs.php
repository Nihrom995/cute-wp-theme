...
<?php if ( ! empty( $items ) ) : ?>
	<div class="breadcrumb hidden-sm-down">
		...
		<?php for ( $i = 0; $i < count( $items ); $i ++ ) : ?>
			<?php if ( $i == ( count( $items ) - 1 ) ) : ?>
				<span class="breadcrumb-item active"><?php echo $items[ $i ]['name'] ?></span>
			<?php elseif ( $i == 0 ) : ?>
				<span class="first-item breadcrumb-item">
			...
			<?php
				...
				<span class="<?php echo( $i - 1 ) ?>-item breadcrumb-item">
					<?php if( isset( $items[ $i ]['url'] ) ) : ?>
						<a href="<?php echo esc_attr($items[ $i ]['url']) ?>"><?php echo $items[ $i ]['name'] ?></a></span>
					<?php else : echo $items[ $i ]['name']; endif ?>
    ...
    <div class="hidden-sm-up mb-4 mb-md-0"></div>
<?php endif ?>